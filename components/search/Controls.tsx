...
function Controls({ page }: { page: ProductListingPage }) {
  ...
  return (
    <Container class="flex flex-col justify-between mb-4 md:mb-0 p-4 md:p-0 sm:gap-4 sm:flex-row sm:h-[53px] md:border-b-1">
      ...
      <div class="flex flex-row sm:gap-4 items-center border-b-1 border-default md:border-none">
        <Sort />
        <Button
          variant="primary"
          class="bg-black w-[50%] relative text-white font-normal text-center uppercase text-[16px] rounded-none"
          onClick={() => {
            open.value = true;
          }}
        >
          Filtrar
          <Icon
            class="absolute right-[16px] top-1/2 translate-y-[-50%]"
            id="FilterList"
            width={16}
            height={16}
          />
